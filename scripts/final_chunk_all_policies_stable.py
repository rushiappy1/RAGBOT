...
PDF_FILES = ["Bajaj Allianz Motor.pdf", "Reliance General Motor.pdf", "HDFC ERGO Motor.pdf", "SBI General Motor.pdf", "ICICI Lombard Motor.pdf", "TATA AIG Motor.pdf"]
...
SECTION_SPLIT_THRESHOLD, TARGET_SPLIT_SIZE, MIN_CHUNK, OVERLAP, SIM_THRESHOLD, HARD_CAP = 800, 550, 150, 60, 0.25, 900
...
def md_blocks(md_text):
    ...
    while i < n:
        ...
        if m: yield ("heading", line.strip()); i += 1; continue
        if MD_TABLE_RE.match(line):
            j = i; buf = []
            while j < n and MD_TABLE_RE.match(lines[j]): buf.append(lines[j]); j += 1
            yield ("table", "\n".join(buf)); i = j; continue
        j = i; buf = []
        while j < n and not MD_HEADING_RE.match(lines[j]) and not MD_TABLE_RE.match(lines[j]):
            if lines[j].strip(): buf.append(lines[j].rstrip())
            j += 1
        para = "\n".join(buf)
...
class StableChunker:
    ...
    @staticmethod
    def est_tokens(text): return max(1, int(len(text) / 4))
    ...
    def sentences_from_block(self, kind, text):
        if kind in ("heading", "table"): return [text.strip()]
        lines = text.split("\n")
        block, cluster = [], []
        for l in lines:
            if MD_BULLET_RE.match(l): cluster.append(l.strip())
            else:
                if cluster: block.append(" ".join(cluster)); cluster = []
                if l.strip(): block.append(l.strip())
        if cluster: block.append(" ".join(cluster))
        out = []
        for seg in block:
            if len(seg) < 300: out.append(seg)
            else:
    ...
    def split_large_section(self, sents, lineage):
        ...
        def anchor(lin): return (lin[-1] + " :: " if lin else "")
        ...
        for idx, s in enumerate(sents):
            ...
            if need_flush and current and (current_tok + est) <= HARD_CAP:
                chunks.append(("\n".join(current).strip(), current_tok))
                current = self.overlap_tail(current)
                current_tok = sum(self.est_tokens(x) for x in current)
            current.append(txt)
    ...
    def chunk(self, md_text, doc_id):
        ...
        for kind, block in md_blocks(md_text):
            if kind == "heading":
                ...
                for s in self.sentences_from_block(kind, block): current_sents.append(s)
        if current_sents: sections.append((current_heading, current_lineage, current_sents))
        def anchor(lin): return (lin[-1] + " :: " if lin else "")
        chunks = []
        for heading, lineage, sents in sections:
            ...
            if section_tok <= SECTION_SPLIT_THRESHOLD: chunks.append((section_text.strip(), section_tok))
            else: chunks.extend(self.split_large_section(section_parts, lineage))
        final_chunks = []
        for chunk_text, chunk_tok in chunks:
            if chunk_tok <= HARD_CAP: final_chunks.append((chunk_text, chunk_tok))
            else:
                lines = chunk_text.split("\n")
                current_buf, current_tok = [], 0
                for line in lines:
                    line_tok = self.est_tokens(line)
                    if current_tok + line_tok > HARD_CAP and current_buf:
                        final_chunks.append(("\n".join(current_buf).strip(), current_tok))
                        current_buf, current_tok = [], 0
                    current_buf.append(line)
                    current_tok += line_tok
                if current_buf: final_chunks.append(("\n".join(current_buf).strip(), current_tok))
        return final_chunks
...
def process_pdf(pdf_path: Path):
    ...
    with open(jsonl_out, "w", encoding="utf-8") as f:
        for i, (chunk, tok) in enumerate(chunks):
            f.write(json.dumps({"doc_id": doc_id, "chunk_id": f"{doc_id}::chunk-{i+1}", "text": chunk, "approx_tokens": tok}, ensure_ascii=False) + "\n")
    print(f"✅ {pdf_path.name}: {len(chunks)} chunks")

def main():
    for name in PDF_FILES:
        pdf_path = PDF_DIR / name
        if not pdf_path.exists(): print(f"⚠️ Missing {pdf_path}"); continue
        process_pdf(pdf_path)
    print("\n✅ Week 0 complete: all chunks frozen")
