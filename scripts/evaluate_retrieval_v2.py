...
def hybrid_search_v2(query, top_k=5, bm25_weight=0.5, vector_weight=0.5):
    ...
    top_candidates = combined_scores[:20]
...
print("="*80)
print("RETRIEVAL EVALUATION V2 - WITH REWRITING + RERANKING")
print("="*80 + "\n")
...
for q in eval_data["queries"]:
    ...
    retrieved = hybrid_search_v2(query_text, top_k=5)
    
    print("  Top-5 chunks:")
    for rank, result in enumerate(retrieved, 1):
        print(f"    [{rank}] {result['chunk_id']} | score={result.get('rerank_score', result['score']):.3f}")
    print()
...
# Save results
with open("data/eval_results_v2.json", "w") as f:
    json.dump(results, f, indent=2)

print("✅ V2 evaluation results saved to data/eval_results_v2.json")
